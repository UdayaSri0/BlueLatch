...
- Targeted maintenance release focused on release pipeline stability and metadata consistency.

### Fixes

- Fixed release workflow failures when GitHub Pages content is not generated by conditionally uploading/deploying Pages only when APT signing is enabled.
- Kept release artifact publishing focused on real build outputs when Pages publication is intentionally skipped.
- Bumped project version references from `0.1.1.8` to `0.1.1.9` across packaging and documentation metadata.
