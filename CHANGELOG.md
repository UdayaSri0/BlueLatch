...
- Improved trusted-device row presentation with safer single-line title/subtitle rendering and consistent state details.
- Reworked Settings page layout with `Gtk.ScrolledWindow` + `Adw.Clamp` to prevent clipped switches/dropdowns/spin buttons near the right edge.
- Standardized control alignment/sizing for switches, dropdowns, and numeric inputs in settings rows.
- Synchronized release metadata and docs to `0.1.2.0` across app version, changelog, Debian packaging, AppStream metadata, and README examples.
